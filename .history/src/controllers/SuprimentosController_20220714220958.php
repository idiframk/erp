...
class SuprimentosController extends Controller
{
    ...
    public function material_addAction()
    {
        ...
        //str_replace($subject,$replace, $value);

        // $estoq_mim = (filter_input(INPUT_POST, 'estoq_mim'));
        $estoq_mim = str_replace($subject, $replace, filter_input(INPUT_POST, 'estoq_mim'));

        // $estoq_max = (filter_input(INPUT_POST, 'estoq_max'));
        $estoq_max = str_replace($subject, $replace, filter_input(INPUT_POST, 'estoq_max'));
        ...
        if (isset($apelido) && !empty($apelido)) {
            ...
            if (count($data) === 0) {
                tbl_produto::insert([
                    ...
                    'Prod_Esoq_min' => $estoq_mim,
                    'Prod_Estoq_max' => $estoq_max,
